-- Never Ever hung vote behavior:
-- If all four options are selected equally, no one is called out.
...
begin
  ...
  select (count(*) = 4 and count(distinct c) = 1)
  into v_is_hung
  from counts;
