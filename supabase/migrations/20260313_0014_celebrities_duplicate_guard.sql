-- Guard against duplicate/near-duplicate celebrity picks across the same lobby.
-- Uses trigram similarity for ~70% matching.
...
as $$
  select case
    ...
    when similarity(p_left, p_right) >= 0.70 then true
    else false
  end;
$$;
