-- Draw WF: when a new guesser joins active guess_live, extend deadline to preserve a full guess window
...
begin
  ...
  then
    ...
    if v_joiner_added then
      update public.draw_wf_rounds
      set guess_deadline_at = greatest(coalesce(guess_deadline_at, now()), now()) + interval '10 seconds'
      where id = v_round.id;
    end if;
