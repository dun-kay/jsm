-- Most Likely pair vote mapping fix:
-- "Me" should map to pair_player_a for both voters
-- "Them" should map to pair_player_b for both voters
-- so me+me = agree, me+them = disagree
...
begin
  ...
  -- Fixed mapping by pair slot, not voter identity.
  if p_choice = 'me' then
    v_target := v_game.pair_player_a_id;
  else
    v_target := v_game.pair_player_b_id;
  end if;
