-- Draw WF: auto-submit only when correct; wrong manual guesses do not submit.
-- Timeout can submit as empty guess to advance waiting state.
...
begin
  ...
  if v_guess <> '' and v_guess <> upper(v_round.word) then
  ...
  values (v_round.id, v_ctx.player_id, v_guess, (v_guess = upper(v_round.word)), true)
